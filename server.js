...
app.use(express.static('.')); // Serve file HTML
...
// GET all borrowings (with book info)
...
// POST create borrowing (pinjam buku)
...
// PUT return book (kembalikan buku)
...
// DELETE borrowing (batal pinjam sebelum dikembalikan)
app.delete('/api/borrowings/:id', async (req, res) => {
  const { id } = req.params;
  try {
    // Hanya boleh hapus jika belum dikembalikan
    const current = await pool.query('SELECT book_id, status FROM borrowings WHERE id = $1', [id]);
    if (current.rows.length === 0) return res.status(404).json({ error: 'Borrowing not found' });
...
app.listen(PORT, () => console.log(`📚 Library system running on port ${PORT}`));